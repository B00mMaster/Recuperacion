...
public class Snake : MonoBehaviour
{
   ...
   public static float gridMoveTimerMax = 0.2f; // La serpiente se moverá a cada segundo
    ...
    private void HandleGridMovement() // Relativo al movimiento en 2D
    {
        ...
        if (gridMoveTimer >= gridMoveTimerMax)
        {
            ...
            {
                if (gridPosition == movePosition.GetGridPosition()&& eatPowerUpBool==false) // Posición de la cabeza coincide con alguna parte del cuerpo
                {
                    // GAME OVER
                    state = State.Dead;
                    GameManager.Instance.SnakeDied();
